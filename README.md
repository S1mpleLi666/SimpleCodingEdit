...
⚡ 安全代码执行：自动运行代码并捕获输出和错误
🔄 自我修复循环：代码出错时自动分析错误并重试修正
...
| Coder | 根据计划和错误信息生成/修复 Python 代码 |
| Executor | 在本地 Python 环境执行代码，捕获 stdout 和异常 |
| Reflector | 评估执行结果，决定结束流程还是返回修复 |
状态设计 
...
BASE_URL=http://你的API地址/v1   # 如果不是使用 OpenAI 官方，需指定

 
示例 — 使用 DeepSeek（国内可直接访问）：
...
from agent import app  # 你的 Agent 图
...
planner ---
plan: 1. 定义斐波那契函数  2. 计算前10项  3. 打印结果
coder ---
code: def fib(n):
    a, b = 0, 1
    for _ in range(n):
...
| 参数 | 位置 | 默认值 | 说明 |
|------|------|--------|------|
| model | ChatOpenAI() | gpt-4 | LLM 模型名称 |
| temperature | ChatOpenAI() | 0 | 生成温度（越低越稳定） |
| MAXITER | shouldcontinue() | 3 | 最大重试次数 |
| base_url | ChatOpenAI() | https://api.openai.com/v1 | API 地址 |
🔧 扩展方向
1. 添加新工具 
...
tools = [
    Tool(name="python_executor", func=PythonExecutor.run, description="执行Python代码"),
    Tool(name="websearch", func=searchfunction, description="搜索网络获取信息"),
    Tool(name="filewriter", func=writefile, description="将代码写入文件"),
]
...
workflow.addedge("coder", "humanreview")
humanreviewnode 中使用 interrupt 等待人工确认
...
agent = createreactagent(llm, tools, react_prompt)
...
A: LangChain 新版已将消息类迁移到 langchaincore，将 from langchain.schema import HumanMessage 改为 from langchaincore.messages import HumanMessage。
